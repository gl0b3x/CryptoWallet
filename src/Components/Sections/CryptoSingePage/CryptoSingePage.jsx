import classes from "./CryptoSingePage.module.css";
        ...
        if (chartResponse.data.length > 0) {
          setInitialData(
            chartResponse.data
              ...
              .filter(
                (entry, index, array) =>
                  index === 0 || entry.time !== array[index - 1].time,
              ),
          );
        }
      } catch (err) {
      ...
      if (
        ...
        toolTip.innerHTML = `
          ...
          <div style="color: ${textColor}">  
            ${
              period === "24h"
                ? new Date(param.time * 1000).toLocaleTimeString("en-US")
...
export default CryptoSingePage;
